...
data$other_features <- data$other_features %>%
  ...
  gsub("(alarm system)([0-9])", "alarm system | \\2",.)


# mi concentro sulla variabile other_features -------------------------------
...
table(data2$kitchen, data2$`only kitchen furnished`)
table(data2$`partially furnished`, 
      data2$furnished)
dt <- cbind(data2$kitchen, data2$`only kitchen furnished`)
colnames(dt) <- c("kitchen", "only kitchen")
sum(data2$`NA`) # 20
sum(data2$fireplace) 
sum(data2$`8 balconies`) # 1
sum(data2$`6 balconies`) # 1
sum(data2$`private and shared garden`)
sum(data2$`property land1 balcony`)

table(data2$`full day concierge`, data2$reception)


(data$other_features)
data %>%
  filter(str_detect(other_features, regex("pvc", ignore_case = TRUE)))
...
data2['concierge/reception'] <- ifelse(data2$reception == 1 |
                                         data2$`full day concierge` == 1 | data2$`half-day concierge`, 1, 0)
...
# creo una variabile che indichi se la casa è esposta a sud o meno
data_exp <- data2 %>%
  dplyr::select(-c(`internal exposure`, `external exposure`, `double exposure`, exposure)) %>%
...
# variabile per il materiale (legno, no legno)
...
# variabile per finestre con un solo vetro o più
...
data2 <- data2 %>%
  dplyr::select(-c(`half-day concierge`, `full day concierge`, reception, 
                   ...
                   `exposure north, south, west`, balcony, terrace, `1 balcony`,
...
hist(log(data$square_meters), freq = F, breaks = 40, col = 'slateblue', 
     main = 'log of square meters histogram')
...
# in vendita dala variabile floor non risultano essere mai sopra al nono piano
data <- data %>%
...
dist_zone = dist_zone %>%
  ...
  mutate(distance_from_duomo_km = ifelse(!is.na(latitude) & !is.na(longitude),
                                         distHaversine(c(longitude, latitude), duomo) / 1000,
                                         NA_real_))
data = data %>%
...
# li metto come NA e li riimputo
data <- data %>% 
...
data <- dplyr::select(data, -c(single_glass, `electric gate`))
...
write.csv(data.frame(data), "test_clean.csv", row.names = FALSE)
